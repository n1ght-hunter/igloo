/// <reference path="./interfaces/iced-app-alignment.d.ts" />
/// <reference path="./interfaces/iced-app-button.d.ts" />
/// <reference path="./interfaces/iced-app-checkbox.d.ts" />
/// <reference path="./interfaces/iced-app-column.d.ts" />
/// <reference path="./interfaces/iced-app-combo-box.d.ts" />
/// <reference path="./interfaces/iced-app-container.d.ts" />
/// <reference path="./interfaces/iced-app-element.d.ts" />
/// <reference path="./interfaces/iced-app-float.d.ts" />
/// <reference path="./interfaces/iced-app-grid.d.ts" />
/// <reference path="./interfaces/iced-app-image.d.ts" />
/// <reference path="./interfaces/iced-app-keyed.d.ts" />
/// <reference path="./interfaces/iced-app-length.d.ts" />
/// <reference path="./interfaces/iced-app-markdown.d.ts" />
/// <reference path="./interfaces/iced-app-message-types.d.ts" />
/// <reference path="./interfaces/iced-app-message.d.ts" />
/// <reference path="./interfaces/iced-app-padding.d.ts" />
/// <reference path="./interfaces/iced-app-pane-grid.d.ts" />
/// <reference path="./interfaces/iced-app-pick-list.d.ts" />
/// <reference path="./interfaces/iced-app-progress-bar.d.ts" />
/// <reference path="./interfaces/iced-app-radio.d.ts" />
/// <reference path="./interfaces/iced-app-row.d.ts" />
/// <reference path="./interfaces/iced-app-rule.d.ts" />
/// <reference path="./interfaces/iced-app-scrollable.d.ts" />
/// <reference path="./interfaces/iced-app-shared.d.ts" />
/// <reference path="./interfaces/iced-app-slider.d.ts" />
/// <reference path="./interfaces/iced-app-space.d.ts" />
/// <reference path="./interfaces/iced-app-svg.d.ts" />
/// <reference path="./interfaces/iced-app-table.d.ts" />
/// <reference path="./interfaces/iced-app-text-input.d.ts" />
/// <reference path="./interfaces/iced-app-text.d.ts" />
/// <reference path="./interfaces/iced-app-toggler.d.ts" />
/// <reference path="./interfaces/iced-app-tooltip.d.ts" />
/// <reference path="./interfaces/iced-app-vertical-slider.d.ts" />
declare module 'iced:app/app@0.1.0' {
  export type Element = import('iced:app/shared@0.1.0').Element;
  export type MessageId = import('iced:app/message@0.1.0').MessageId;
  export type Message = import('iced:app/message@0.1.0').Message;
  export type * as IcedAppAlignment010 from 'iced:app/alignment@0.1.0'; // import iced:app/alignment@0.1.0
  export type * as IcedAppButton010 from 'iced:app/button@0.1.0'; // import iced:app/button@0.1.0
  export type * as IcedAppCheckbox010 from 'iced:app/checkbox@0.1.0'; // import iced:app/checkbox@0.1.0
  export type * as IcedAppColumn010 from 'iced:app/column@0.1.0'; // import iced:app/column@0.1.0
  export type * as IcedAppComboBox010 from 'iced:app/combo-box@0.1.0'; // import iced:app/combo-box@0.1.0
  export type * as IcedAppContainer010 from 'iced:app/container@0.1.0'; // import iced:app/container@0.1.0
  export type * as IcedAppElement010 from 'iced:app/element@0.1.0'; // import iced:app/element@0.1.0
  export type * as IcedAppFloat010 from 'iced:app/float@0.1.0'; // import iced:app/float@0.1.0
  export type * as IcedAppGrid010 from 'iced:app/grid@0.1.0'; // import iced:app/grid@0.1.0
  export type * as IcedAppImage010 from 'iced:app/image@0.1.0'; // import iced:app/image@0.1.0
  export type * as IcedAppKeyed010 from 'iced:app/keyed@0.1.0'; // import iced:app/keyed@0.1.0
  export type * as IcedAppLength010 from 'iced:app/length@0.1.0'; // import iced:app/length@0.1.0
  export type * as IcedAppMarkdown010 from 'iced:app/markdown@0.1.0'; // import iced:app/markdown@0.1.0
  export type * as IcedAppMessageTypes010 from 'iced:app/message-types@0.1.0'; // import iced:app/message-types@0.1.0
  export type * as IcedAppMessage010 from 'iced:app/message@0.1.0'; // import iced:app/message@0.1.0
  export type * as IcedAppPadding010 from 'iced:app/padding@0.1.0'; // import iced:app/padding@0.1.0
  export type * as IcedAppPaneGrid010 from 'iced:app/pane-grid@0.1.0'; // import iced:app/pane-grid@0.1.0
  export type * as IcedAppPickList010 from 'iced:app/pick-list@0.1.0'; // import iced:app/pick-list@0.1.0
  export type * as IcedAppProgressBar010 from 'iced:app/progress-bar@0.1.0'; // import iced:app/progress-bar@0.1.0
  export type * as IcedAppRadio010 from 'iced:app/radio@0.1.0'; // import iced:app/radio@0.1.0
  export type * as IcedAppRow010 from 'iced:app/row@0.1.0'; // import iced:app/row@0.1.0
  export type * as IcedAppRule010 from 'iced:app/rule@0.1.0'; // import iced:app/rule@0.1.0
  export type * as IcedAppScrollable010 from 'iced:app/scrollable@0.1.0'; // import iced:app/scrollable@0.1.0
  export type * as IcedAppShared010 from 'iced:app/shared@0.1.0'; // import iced:app/shared@0.1.0
  export type * as IcedAppSlider010 from 'iced:app/slider@0.1.0'; // import iced:app/slider@0.1.0
  export type * as IcedAppSpace010 from 'iced:app/space@0.1.0'; // import iced:app/space@0.1.0
  export type * as IcedAppSvg010 from 'iced:app/svg@0.1.0'; // import iced:app/svg@0.1.0
  export type * as IcedAppTable010 from 'iced:app/table@0.1.0'; // import iced:app/table@0.1.0
  export type * as IcedAppTextInput010 from 'iced:app/text-input@0.1.0'; // import iced:app/text-input@0.1.0
  export type * as IcedAppText010 from 'iced:app/text@0.1.0'; // import iced:app/text@0.1.0
  export type * as IcedAppToggler010 from 'iced:app/toggler@0.1.0'; // import iced:app/toggler@0.1.0
  export type * as IcedAppTooltip010 from 'iced:app/tooltip@0.1.0'; // import iced:app/tooltip@0.1.0
  export type * as IcedAppVerticalSlider010 from 'iced:app/vertical-slider@0.1.0'; // import iced:app/vertical-slider@0.1.0
  export * as message from 'iced:app/message@0.1.0'; // export iced:app/message@0.1.0
  export function update(messageId: MessageId, message: Message): void;
  /**
  * Exported function for creating a view
  */
  export function view(): Element;
}
